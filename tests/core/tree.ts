...
function mergeStepNodes(tree, steps) {
    for (let i = 0; i < steps.length; i++) {
        const stepNode = tree.stepNodeIndex[steps[i].id];
...
`,
                ...
                assert.throws(() => {
                    tree.parseIn('Code block here {', 'file.txt');
...
* My function
    Step one -
            ...
            const branchAbove = new Branch();
            const functionCall = new Step(tree.root.children[0].id);
            const functionDeclarations = tree.findFunctionDeclarations(functionCall, branchAbove);
...
`);
            ...
            const branchAbove = new Branch();
            const functionCall = new Step(tree.root.children[1].id);
            const functionDeclarations = tree.findFunctionDeclarations(functionCall, branchAbove);
...
`);
            ...
            const branchAbove = new Branch();
            const functionCall = new Step(tree.root.children[0].children[1].id);
            const functionDeclarations = tree.findFunctionDeclarations(functionCall, branchAbove);
...
`);
            ...
            const branchAbove = new Branch();
            branchAbove.steps = [new Step(tree.root.children[0].id)];
            const functionCall = new Step(tree.root.children[0].children[0].id);
            const functionDeclarations = tree.findFunctionDeclarations(functionCall, branchAbove);
...
`);
            ...
            const branchAbove = new Branch();
            branchAbove.steps = [new Step(tree.root.children[0].steps[0].id)];
            const functionCall = new Step(tree.root.children[0].children[0].id);
            const functionDeclarations = tree.findFunctionDeclarations(functionCall, branchAbove);
...
* F
            ...
            const equivalents = tree.equivalents(tree.root.children[0]); // * F

            Comparer.expect(equivalents).to.match([{ text: 'F', lineNumber: 2 }]);
        });
...
A -
    ...
    * F
            ...
            const equivalents = tree.equivalents(tree.root.children[0].children[1]); // * F
...
A -
    * F
            ...
            const equivalents = tree.equivalents(tree.root.children[0].children[0]); // * F

            Comparer.expect(equivalents).to.match([{ text: 'F', lineNumber: 3 }]);
        });
...
* F
    ...
    * G
            ...
            const equivalents = tree.equivalents(tree.root.children[0].children[0]); // * G
...
* F
    ...
    * G
        * H
            ...
            const equivalents = tree.equivalents(tree.root.children[0].children[0].children[0]); // * H
    ...
    describe('branchify()', () => {
        context('generic tests', () => {
            it('handles an empty tree', () => {
                ...
                const branches = tree.branchify(tree.root);
...
G -
                ...
                const nonParallelId = branches[1].nonParallelIds[0];
...
F
                ...
                const nonParallelId = branches[0].nonParallelIds[0];
...
G -
                ...
                const nonParallelId0 = branches[0].nonParallelIds[0];
                const nonParallelId1 = branches[1].nonParallelIds[0];
            ...
            it.skip('throws an exception when there\'s an infinite loop among function calls', function () {
                // function() needed for this.timeout() to work
                this.timeout(10000);
...
*** Before Every Step {
    G
                ...
                Comparer.expect(branches).to.match([
                    {
                        steps: [{ text: 'A' }],
                        beforeEveryBranch: [{ text: 'Before Every Branch', codeBlock: '\n    F', isPackaged: true }],
                        afterEveryBranch: [{ text: 'After Every Branch', codeBlock: '\n    D', isPackaged: true }],
                        beforeEveryStep: [{ text: 'Before Every Step', codeBlock: '\n    G', isPackaged: true }],
                        afterEveryStep: [{ text: 'After Every Step', codeBlock: '\n    E', isPackaged: true }]
                    }
                ]);
                ...
                Comparer.expect(tree).to.match({
                    ...
                    afterEverything: [{ text: 'After Everything', codeBlock: '\n    C', isPackaged: true }]
                });
            });
        });
...
* F
    * G
    ...
    G
                    ...
                    Comparer.expect(branches).to.match([
                        {
                            steps: [{ text: 'F' }, { text: 'G' }, { text: 'H' }, { text: 'One' }]
                        }
                    ]);
...
- Seq ..
    ...
    G
                    ...
                    Comparer.expect(branches).to.match([
                        {
                            steps: [{ text: 'Seq' }, { text: 'F' }, { text: 'G' }, { text: 'H' }]
                        }
                    ]);
                });
    ...
    - B
        * G
            - D
                    ...
                    Comparer.expect(branches).to.match([
                        {
                            steps: [{ text: 'F' }, { text: 'A' }, { text: 'G' }, { text: 'C' }]
                        },
                        {
                            steps: [{ text: 'F' }, { text: 'B' }, { text: 'G' }, { text: 'D' }]
                        }
                    ]);
                });
...
* F
    Two -
                    ...
                    Comparer.expect(branches).to.match([
                        {
                            steps: [{ text: 'F' }, { text: 'One' }]
                        },
                        {
                            steps: [{ text: 'F' }, { text: 'Two' }]
                        }
                    ]);
                });
...
* F
    * G
        Two -
                    ...
                    Comparer.expect(branches).to.match([
                        {
                            steps: [{ text: 'F' }, { text: 'G' }, { text: 'One' }]
                        },
                        {
                            steps: [{ text: 'F' }, { text: 'G' }, { text: 'Two' }]
                        }
                    ]);
                });
    ...
    * G
        Four -
                    ...
                    Comparer.expect(branches).to.match([
                        ...
                        {
                            steps: [{ text: 'F' }, { text: 'G' }, { text: 'Three' }]
                        },
                        {
                            steps: [{ text: 'F' }, { text: 'G' }, { text: 'Four' }]
                        }
                    ]);
                });
...
* H
    J -
                    ...
                    Comparer.expect(branches).to.match([
                        {
                            steps: [{ text: 'F' }, { text: 'H' }, { text: 'J' }, { text: 'G' }, { text: 'One' }]
                        },
                        {
                            steps: [{ text: 'F' }, { text: 'H' }, { text: 'J' }, { text: 'G' }, { text: 'Two' }]
                        },
                        {
                            steps: [{ text: 'F' }, { text: 'H' }, { text: 'J' }, { text: 'G' }, { text: 'Three' }]
                        },
                        {
                            steps: [{ text: 'F' }, { text: 'H' }, { text: 'J' }, { text: 'G' }, { text: 'Four' }]
                        }
                    ]);
                });
        ...
        * H
            Seven -
                    ...
                    Comparer.expect(branches).to.match([
                        ...
                        {
                            steps: [{ text: 'F' }, { text: 'G' }, { text: 'H' }, { text: 'Two' }]
                        },
                        {
                            steps: [{ text: 'F' }, { text: 'G' }, { text: 'H' }, { text: 'Three' }]
                        },
                        {
                            steps: [{ text: 'F' }, { text: 'G' }, { text: 'H' }, { text: 'Four' }]
                        },
                        {
                            steps: [{ text: 'F' }, { text: 'G' }, { text: 'H' }, { text: 'Five' }]
                        },
                        {
                            steps: [{ text: 'F' }, { text: 'G' }, { text: 'H' }, { text: 'Six' }]
                        },
                        {
                            steps: [{ text: 'F' }, { text: 'G' }, { text: 'H' }, { text: 'Seven' }]
                        }
                    ]);
                });
...
* F
    Two -
                    ...
                    Comparer.expect(branches).to.match([
                        {
                            steps: [{ text: 'Test' }, { text: 'F' }, { text: 'One' }]
                        },
                        {
                            steps: [{ text: 'Test' }, { text: 'F' }, { text: 'Two' }]
                        }
                    ]);
                });
...
* Mobile
    * On cart page
        Four -
                    ...
                    Comparer.expect(branches).to.match([
                        {
                            steps: [{ text: 'Desktop' }, { text: 'On cart page' }, { text: 'Three' }]
                        }
                    ]);
                });
...
* F
    Four -
                    ...
                    Comparer.expect(branches).to.match([
                        ...
                        {
                            steps: [{ text: 'A' }, { text: 'B' }, { text: 'F' }, { text: 'Two' }]
                        }
                    ]);
                });
...
A
    B
        C
                    ...
                    Comparer.expect(branches).to.match([
                        {
                            steps: [{ text: 'A' }, { text: 'B' }, { text: 'C' }, { text: 'One' }]
                        }
                    ]);
    ...
    B
        C
            D
                    ...
                    Comparer.expect(branches).to.match([
                        {
                            steps: [{ text: 'A' }, { text: 'B' }, { text: 'C' }, { text: 'D' }, { text: 'Two' }]
                        }
                    ]);
                });
    ...
    B
        C
                    ...
                    Comparer.expect(branches).to.match([
                        ...
                        {
                            steps: [{ text: 'A' }, { text: 'B' }, { text: 'C' }, { text: 'Two' }]
                        }
                    ]);
                });
            });
...
* F3 {
    return '5';
                ...
                Comparer.expect(branches).to.match([
                    {
                        steps: [{ text: '{var} = F' }, { text: '{var}=\'1\'' }, { text: '{var2}=\'0\'' }]
                    },
                    {
                        steps: [
                            { text: '{var} = F' },
                    ...
                    {
                        steps: [{ text: '{var} = F' }, { text: '{var}=\'4\'' }, { text: '{var2}=\'0\'' }]
                    },
                    {
                        steps: [{ text: '{var} = F' }, { text: '{var}=F3' }, { text: '{var2}=\'0\'' }]
                    }
                ]);
...
A - ..
                ...
                Comparer.expect(branches).to.match([
                    {
                        steps: [{ text: 'A', level: 0, isSequential: true }]
                    }
                ]);
            });
...
E -
                ...
                Comparer.expect(branches).to.match([
                    ...
                    {
                        steps: [{ text: 'A' }, { text: 'C' }],
                        beforeEveryBranch: [{ text: 'Before Every Branch', level: 0, codeBlock: '\n            D' }],
                        afterEveryBranch: undefined,
                        beforeEveryStep: undefined,
                        afterEveryStep: undefined
                    },
...
E -
                ...
                Comparer.expect(branches).to.match([
                    ...
                    },
                    {
                        steps: [{ text: 'A' }, { text: 'C' }],
                        beforeEveryBranch: undefined,
                        afterEveryBranch: [{ text: 'After Every Branch', level: 0, codeBlock: '\n            D' }],
                        beforeEveryStep: undefined,
                        afterEveryStep: undefined
                    },
                    {
...
E -
                ...
                Comparer.expect(branches).to.match([
                    ...
                    },
                    {
                        ...
                        beforeEveryBranch: undefined,
                        afterEveryBranch: undefined,
                        beforeEveryStep: [{ text: 'Before Every Step', level: 0, codeBlock: '\n            D' }],
                        afterEveryStep: undefined
                    },
                    {
                        steps: [{ text: 'E' }],
...
E -
                ...
                Comparer.expect(branches).to.match([
                    ...
                    },
                    {
                        ...
                        afterEveryBranch: undefined,
                        beforeEveryStep: undefined,
                        afterEveryStep: [{ text: 'After Every Step', level: 0, codeBlock: '\n            D' }]
                    },
                    {
                        steps: [{ text: 'E' }],
                        beforeEveryBranch: undefined,
...
*** After Everything {
                ...
                mergeStepNodes(tree, tree.afterEverything);
                Comparer.expect(tree.afterEverything).to.match([{ text: 'After Everything', level: 0, codeBlock: '' }]);
            });
...
F ~
                ...
                Comparer.expect(tree.branches).to.match([
                    {
                        steps: [{ text: 'F', isAfterDebug: true }]
                    }
                ]);
            });
        });
...
K-1 -
    ...
    `,
                ...
                console.log(
                    'Size of serialized tree: ' + JSON.stringify(tree.serialize()).length / (1024 * 1024) + ' MB'
                );
            });
        });
    });
...
`);
    ...
    describe('serializeSnapshot()', () => {
        it('outputs a snapshot for an empty tree', () => {
            const tree = new Tree();
            const snapshot = tree.serializeSnapshot();
...
G -
    H -
            ...
            const snapshot = tree.serializeSnapshot(2);
...
G -
    H -
            ...
            const prevSnapshot = tree.serializeSnapshot();
            const snapshot = tree.serializeSnapshot(undefined, prevSnapshot);
...
G -
    H -
            ...
            const prevSnapshot = tree.serializeSnapshot(2);
            const snapshot = tree.serializeSnapshot(2, prevSnapshot);
...
G -
    H -
            ...
            const snapshot1 = tree.serializeSnapshot(); // includes all 4 branches, branches 0-3 are running

            delete tree.branches[0].isRunning;
            delete tree.branches[1].isRunning;

            const snapshot2 = tree.serializeSnapshot(undefined, snapshot1); // includes all 4 branches, branches 2-3 are running

            delete tree.branches[2].isRunning;

            const snapshot3 = tree.serializeSnapshot(undefined, snapshot2); // includes branches 2-3, branch 3 is running
...
G -
    ...
    describe('markPassedFromPrevRun()', () => {
        it('merges empty previous branches into empty current branches', () => {
            const prevTree = new Tree();
            const currTree = new Tree();

            currTree.generateBranches();
            prevTree.generateBranches();
            const previous = prevTree.serializePassed();
        ...
        it('handles a merge', () => {
            ...
            Comparer.expect(currTree).to.match({
                branches: [
                    {
                        steps: [
                            { text: '1A clone-1 step-1' },
                            { text: '1A clone-1 step-2' },
                            { text: '1A clone-1 step-3' }
                        ],
                    ...
                    {
                        steps: [
                            { text: '1A clone-2 step-1' },
                            { text: '1A clone-2 step-2' },
                            { text: '1A clone-2 step-3' }
                        ],
                        passedLastTime: undefined,
                        isPassed: undefined,
                        isFailed: undefined
                    ...
                    {
                        steps: [
                            { text: '1B clone-1 step-1' },
                            { text: '1B clone-1 step-2' },
                            { text: '1B clone-1 step-3' }
                        ],
                        passedLastTime: true,
                        isPassed: undefined,
                        isFailed: undefined
                    ...
                    {
                        steps: [
                            { text: '3 clone-1 step-1' },
                            { text: '3 clone-1 step-2' },
                            { text: '3 clone-1 step-3' }
                        ],
                        passedLastTime: undefined,
                        isPassed: undefined,
                        isFailed: undefined
                    ...
                    {
                        steps: [
                            { text: '3 clone-2 step-1' },
                            { text: '3 clone-2 step-2' },
                            { text: '3 clone-2 step-3' }
                        ],
                        passedLastTime: undefined,
                        isPassed: undefined,
                        isFailed: undefined
                    ...
                    {
                        steps: [
                            { text: '3 clone-3 step-1' },
                            { text: '3 clone-3 step-2' },
                            { text: '3 clone-3 step-3' }
                        ],
                        passedLastTime: undefined,
                        isPassed: undefined,
                        isFailed: undefined
...
`,
    ...
    describe('findSimilarBranches()', () => {
        it('handles empty branches', () => {
            ...
            const similarBranches = tree.findSimilarBranches(branch1, 1);
            Comparer.expect(similarBranches).to.match([{ steps: [] }]);
        });
        ...
        it('finds similar branches', () => {
            ...
            const A = 1;
            tree.stepNodeIndex[A].text = 'A';
            const B = 2;
            tree.stepNodeIndex[B].text = 'B';
            const C = 3;
            tree.stepNodeIndex[C].text = 'C';
            const D = 4;
            tree.stepNodeIndex[D].text = 'D';

            const branch1 = new Branch();
            const branch2 = new Branch();
            const branch3 = new Branch();
            const branch4 = new Branch();
    ...
    describe('nextStep()', () => {
        ...
        it('returns the first step if nothing is running yet, without advancing', () => {
            ...
            const s = tree.nextStep(branch, false);
        ...
        it('returns the next step if one is currently running, without advancing', () => {
            ...
            const s = tree.nextStep(branch, false, false);
            Comparer.expect(s).to.match({
                id: 2,
                isRunning: undefined
            });
        ...
        it('returns the first step if nothing is running yet, with advancing', () => {
            ...
            const s = tree.nextStep(branch, true);
            Comparer.expect(s).to.match({
                id: 1,
                isRunning: true
            });
        ...
        it('ends the branch if the next step is a .s', () => {
            ...
            const s = tree.nextStep(branch1, true);
            Comparer.expect(s).to.match(null);
