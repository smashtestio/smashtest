...
describe('Branch', () => {
    describe('mergeToEnd()', () => {
        ...
        it('merges hook arrays', () => {
            ...
            const stepD = new Step();
            stepD.text = 'D';

            const stepE = new Step();
            stepE.text = 'E';

            const stepF = new Step();
            stepF.text = 'F';

            const stepG = new Step();
            stepG.text = 'G';
            ...
            Comparer.expect(branch1).to.match({
                steps: [{ text: 'A' }, { text: 'B' }, { text: 'C' }],
                beforeEveryBranch: [{ text: 'F' }, { text: 'G' }, { text: 'E' }],
                afterEveryBranch: [{ text: 'E' }, { text: 'F' }, { text: 'G' }],
                beforeEveryStep: [{ text: 'E' }, { text: 'G' }, { text: 'F' }],
                afterEveryStep: [{ text: 'G' }, { text: 'E' }, { text: 'F' }]
            });
        });
    });
    ...
    describe('output()', () => {
        it('outputs the right text', () => {
            const stepNodes = [new StepNode(0), new StepNode(1), new StepNode(2), new StepNode(3)];
            const steps = [new Step(0), new Step(1), new Step(2)];
...
`
    ...
    describe('updateHash()', () => {
        it('generates the hash for the branch', () => {
            const stepNodes = [new StepNode(0), new StepNode(1), new StepNode(2)];
            const steps = [new Step(0), new Step(1), new Step(2)];
