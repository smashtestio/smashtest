 ...
 */
exports.comparison = (value, criteria) => {
    let errors = [];
    ...
    if(typeof criteria == 'object') {
        if(criteria instanceof Array) {
            ...
            if(valueExpectedToBePlainArray) {
                if(!(value instanceof Array)) {
                    ...
                    for(let i = 0; i < criteria.length - 1; i++) {
                        value[i] = exports.comparison(value[i], criteria[i]);
                    }
                }
            }
        }
            ...
            if(criteria.$not) {
                let ret = exports.comparison(value, criteria.$not);
            ...
            if(criteria.$subset) {
                if(typeof value != 'object') {
                    errors.push(`isn't an object so can't match $subset ${criteria.$subset}`);
                }
                else {
                        ...
                        if(criteria.hasOwnProperty(key)) {
                            if(!key.startsWith('$')) {
                                if(!value.hasOwnProperty(key)) {
                                    errors.push(`doesn't have key '${key}' needed to match $subset`);
                                }
                                else {
                                    value[key] = exports.comparison(value[key], criteria[key]);
                                }
                            }
                            else if(key.startsWith("$regex ")) {
                                let ret = exports.regexKeyComparison(value, criteria, key);
            ...
            if(criteria.$every) {
                ...
                if(typeof value != 'object') {
                    ...
                    for(let key in value) {
                        if(value.hasOwnProperty(key)) {
                            value[key] = exports.comparison(value[key], criteria.$every);
                        }
                    }
                }
 ...
 */
exports.regexKeyComparison(value, criteria, key) {
    let errors = [];
    ...
    for(let valueKey in value) {
        if(valueKey.match(regex) && value.hasOwnProperty(valueKey)) {
            // Found a match
            value[valueKey] = exports.comparison(value[valueKey], criteria[key]);
            found = true;
        }
    }
...
exports.print = (obj, indents) => {
    ...
    if(typeof obj.value == 'object') {
        if(obj.value instanceof Array) {
            ...
            let outputted = false;
            for(let item of obj.value) {
                ret += nextSpaces + exports.print(item, indents + 1) + ',\n';
                outputted = true;
            }
            ...
            for(let key in obj.value) {
                if(obj.value.hasOwnProperty(key)) {
                    ret += nextSpaces + key + ': ' + exports.print(obj.value[key], indents + 1) + ',\n';
                    outputted = true;
                }
            }
